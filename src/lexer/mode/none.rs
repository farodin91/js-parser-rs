...
fn start_punctuator(state: &mut LexerState, t: Punctuator) {
    state.mode = LexerMode::Punctuator(t);
}
...
pub fn exec(state: &mut LexerState, c: Option<char>) -> bool {
    match c {
        ...
        Some(' ') | Some('\n') => (),
        Some(';') => state.tokens.push(TokenType::Punctuator(Punctuator::Semicolon)),
        ...
        Some('.') => state.tokens.push(TokenType::Punctuator(Punctuator::Point)),
